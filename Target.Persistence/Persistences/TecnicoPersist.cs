...
namespace Target.Persistence.Persistences
{
    public class TecnicoPersist : ITecnicoPersist
    {
        ...
        public async Task<List<TecnicoDto>> ObterListaTecnicosAsync(string filter)
        {
            ...
            query = !string.IsNullOrEmpty(filter) 
                    ? query.Where(tecnico => tecnico.OccupationArea.Contains(filter) || 
                                            tecnico.ProfessionalQualification.Contains(filter) ||
                                            tecnico.Name.Contains(filter)) 
                    : query;
