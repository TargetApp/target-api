...
namespace Target.API.Controllers
{
    ...
    public class TargetRelatorioController : ControllerBase
    {
        ...
        [HttpGet("mes/{mesId}")]
        public async Task<IActionResult> ObterAnalisesPorMes(int mesId)
        {
            try
            {
                var usuarioId = _usuarioLogado.ObterUsuarioId();
                var reportDetails = await _relatorioService.ObterAnalisesPorMes(usuarioId, mesId);
                if (reportDetails == null) return NotFound("Relatório não encontrado.");
        ...
        [HttpGet("meus-relatorios/{ano}/{mes}/{dia}")]
        public async Task<IActionResult> ObterRelatoriosPorData(int ano, int mes, int dia)
        {
            try
            {
                var usuarioId = _usuarioLogado.ObterUsuarioId();
                var relatorios = await _relatorioService.ObterRelatoriosPorDataAsync(ano, mes, dia, usuarioId);
                if (relatorios.Count == 0) return NotFound("Nenhum relatório encontrado.");
        ...
        public async Task<IActionResult> ObterRelatorioPorId(int relatorioId)
        {
            try
            {
                ...
                if (relatorio == null) return StatusCode(503, "Serviço indisponível. Tente novamente mais tarde.");
